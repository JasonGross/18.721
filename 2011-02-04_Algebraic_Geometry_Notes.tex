...
\usepackage{etextools,pdftexcmds,verbatim,ifplatform,catchfile}
\input{header}

\makeatletter
% \newwrite\@tempwritea
% \newif\if@shellescape
% \edef\@testfilename{\jobname.18}% fix; this gets *s?!!!!
...
% }
\newcommand{\Ifshellescape}{\ifshellescape\expandafter\@firstoftwo\else\expandafter\@secondoftwo\fi} %\if@shellescape\expandafter\@firstoftwo\else\expandafter\@secondoftwo\fi}
...
\edef\rmcommand{\ifwindows del \else rm \fi}
\edef\copyfcommand{\ifwindows copy /y \else cp -f \fi}
\edef\movecommand{\ifwindows move \else mv \fi}
\edef\startscriptcommand{\ifwindows start \else bash \fi}
\newcommand{\writeallcompiles}{%
  ...
  \forcsvloop\filelist\do{%
    ...
    \immediate\write\compileout{(\rmcommand "##1.pdf") & (pdflatex "##1.tex") && (\rmcommand "##1.pdf") && (pdflatex "##1.tex")}%
    \immediate\write\compileout{\copyfcommand "##1.tex" "##1.tex.old"}%
  }%
  \immediate\write\compileout{popd}%
  \immediate\closeout\compileout
  \immediate\write18{\movecommand compile-all.tex compile-all.bat}%
...
\newcommand{\compileallfiles}{
  ...
  \immediate\write18{\copyfcommand *-*-*_Diagram_*.pdf "Notes by Day/"}%
  \message{Writing compilation routines...^^J}%
  \writeallcompiles
  \immediate\write18{echo Compiling files...}%^^J}
  \immediate\write18{\startscriptcommand compile-all.bat}%
}
\AtBeginDocument{\AtEndDocument{\immediate\write18{\rmcommand "\jobname.sec"}}}
...
%   \IfFileExists{#1}{%
%     \IfFileExists{#2}{%
%       \CatchFileDef{\@firstfile}{#1}{\let\do\@makeother\dospecials}
%       \CatchFileDef{\@secondfile}{#2}{\let\do\@makeother\dospecials}
...
\begin{notessection}{2}{9}{2011}
  \subsection{Curves in \texorpdfstring{$\mathbb P^2$}{ℙ²}}
    ...
    \begin{defn*}
      If all the $f_i$ vanish at $p$, then $p$ is called a \emph{singular} point of $C = V(f)$.  Otherwise, say that $C$ is \emph{non-singular} at $p$.  Say that $C$ is a \emph{non-singular curve} if it has no singular points.
    \end{defn*}
    
    \subsubsection{Nonsingular curves}
      e.g. 1, an irreducible conic is always non-singular
      \begin{proof}
        Convert to $x^2 - yz = 0$.  $f_x = 2x$, $f_y = -z$, $f_z = -y$.  Since $(x, y, z)\in \mathbb P^2$, not all these can be zero, so it's nonsingular
      \end{proof}
...
\begin{notessection}{4}{4}{2011}
  ...
  %$uv = w \longleftrightarrow \text{point }(a_{ij})$
...
\begin{notessection}{4}{11}{2011}
  \begin{center}\includegraphics[width=\textwidth]{2011-04-11_Diagram_001}\end{center}
  \begin{center}\includegraphics[width=\textwidth]{2011-04-11_Diagram_002}\end{center}
  \begin{center}\includegraphics[width=\textwidth]{2011-04-11_Diagram_003}\end{center}
  \begin{center}\includegraphics[width=\textwidth]{2011-04-11_Diagram_004}\end{center}
\end{notessection}
