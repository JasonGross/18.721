...
\newcommand{\writeallcompiles}{
  ...
  \expandafter\ifx\expandafter\relax\the\filelist
  \else
...
\newcommand{\@appfileto}[1]{%
  \read\readfile@in to\next
  \ifeof\readfile@in
  \else
    \expandafter\appto\expandafter#1\expandafter{\next}%
    \expandaftercmds{\@appfileto{#1}}%
  \fi
}
...
\newenvironment{notessection}[3]{%
  ...
  \writeheader\notes@section@write%
  \begingroup% Lets Keep the Changes Local
    \ExpandNext{\def\temp}{\expandnext{\expandnext{\noexpand\noexpand\setcounter{section}}{\the\value{section}}\section}{\sectiontext}}%
    \notes@pre@writetothis
    \@bsphack
    \let\do\@makeother\dospecials\catcode`\^^M\active
    \def\verbatim@processline{%
