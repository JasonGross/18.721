...
\newcommand{\writeallcompiles}{
  ...
  \immediate\write\compileout{\expandafter\@gobble\detokenize{##!/bin/bash}}
...
%================ writing to this ===============
\newcommand{\notes@pre@writetothis}{\gdef\notes@curcode{}}%\immediate\openout\esgpset@tempout\esgpset@tempfilename
...
\newenvironment{preamble}{%
  \begingroup% Lets Keep the Changes Local
    ...
    \verbatim@start
}{\@esphack\endgroup\global\let\notes@header=\notes@curcode\aftergroup\notes@post@writetothis\relax}
...
\newenvironment{notessection}[3]{%
  ...
  \begingroup% Lets Keep the Changes Local
    ...
    \temp}%
{\immediate\write\notes@section@write{\detokenize{\end{document}}}%
 \immediate\closeout\notes@section@write\@esphack\endgroup%
 \compilefilepart{\notes@filenamepart}%
 \aftergroup\notes@post@writetothis\relax
}%
...
\begin{notessection}{2}{25}{2011}
  ...
  \end{itemize}
  \begin{proof}
    Suppose the theorem is false for some $A$ and some $I$.  Let $S = \mathset{\text{ideals }J\text{ of }A\text{ such that }\rad J\text{ is not a finite intersection of prime ideals}}$.  By hypothesis, $S \ne \emptyset$.  Then there exists a maximal element $I$ in $S$.  Then $\rad I$ not a finite intersection of prime ideals, but every larger ideal is a finite intersection of prime ideals.  
...
\begin{notessection}{3}{7}{2011}
  %\begin{cor*}
    %If $M$ is a finite module, and $(v_1, \ldots, v_r$ are elements of $M$.
  %\end{cor*}
  %\begin{proof}
  %\end{proof}
